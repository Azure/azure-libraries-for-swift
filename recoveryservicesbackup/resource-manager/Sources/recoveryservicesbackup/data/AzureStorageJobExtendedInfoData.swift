...
internal struct AzureStorageJobExtendedInfoData : AzureStorageJobExtendedInfoProtocol {
    public var tasksList: [AzureStorageJobTaskDetailsProtocol?]?
  ...
  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: CodingKeys.self)
      if container.contains(.tasksList) {
        self.tasksList = try container.decode([AzureStorageJobTaskDetailsData?]?.self, forKey: .tasksList)
    }
...
extension DataFactory {
  public static func createAzureStorageJobExtendedInfoProtocol() -> AzureStorageJobExtendedInfoProtocol {
    return AzureStorageJobExtendedInfoData()
  }
}

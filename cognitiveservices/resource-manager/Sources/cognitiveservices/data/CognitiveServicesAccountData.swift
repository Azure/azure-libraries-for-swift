// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
//
// Code generated by Microsoft (R) AutoRest Code Generator.
import Foundation
import azureSwiftRuntime
internal struct CognitiveServicesAccountData : CognitiveServicesAccountProtocol {
    public var etag: String?
    public var id: String?
    public var kind: String?
    public var location: String?
    public var name: String?
    public var properties: CognitiveServicesAccountPropertiesProtocol?
    public var sku: SkuProtocol?
    public var tags: [String:String]?
    public var type: String?

        enum CodingKeys: String, CodingKey {case etag = "etag"
        case id = "id"
        case kind = "kind"
        case location = "location"
        case name = "name"
        case properties = "properties"
        case sku = "sku"
        case tags = "tags"
        case type = "type"
        }

  public init()  {
  }

  public init(from decoder: Decoder) throws {
    let container = try decoder.container(keyedBy: CodingKeys.self)
      if container.contains(.etag) {
        self.etag = try container.decode(String?.self, forKey: .etag)
    }
    if container.contains(.id) {
        self.id = try container.decode(String?.self, forKey: .id)
    }
    if container.contains(.kind) {
        self.kind = try container.decode(String?.self, forKey: .kind)
    }
    if container.contains(.location) {
        self.location = try container.decode(String?.self, forKey: .location)
    }
    if container.contains(.name) {
        self.name = try container.decode(String?.self, forKey: .name)
    }
    if container.contains(.properties) {
        self.properties = try container.decode(CognitiveServicesAccountPropertiesData?.self, forKey: .properties)
    }
    if container.contains(.sku) {
        self.sku = try container.decode(SkuData?.self, forKey: .sku)
    }
    if container.contains(.tags) {
        self.tags = try container.decode([String:String]?.self, forKey: .tags)
    }
    if container.contains(.type) {
        self.type = try container.decode(String?.self, forKey: .type)
    }
    if var pageDecoder = decoder as? PageDecoder  {
      if pageDecoder.isPagedData,
        let nextLinkName = pageDecoder.nextLinkName {
          pageDecoder.nextLink = try UnknownCodingKey.decodeStringForKey(decoder: decoder, keyForDecode: nextLinkName)
      }
    }
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: CodingKeys.self)
    if self.etag != nil {try container.encode(self.etag, forKey: .etag)}
    if self.id != nil {try container.encode(self.id, forKey: .id)}
    if self.kind != nil {try container.encode(self.kind, forKey: .kind)}
    if self.location != nil {try container.encode(self.location, forKey: .location)}
    if self.name != nil {try container.encode(self.name, forKey: .name)}
    if self.properties != nil {try container.encode(self.properties as! CognitiveServicesAccountPropertiesData?, forKey: .properties)}
    if self.sku != nil {try container.encode(self.sku as! SkuData?, forKey: .sku)}
    if self.tags != nil {try container.encode(self.tags, forKey: .tags)}
    if self.type != nil {try container.encode(self.type, forKey: .type)}
  }
}

extension DataFactory {
  public static func createCognitiveServicesAccountProtocol() -> CognitiveServicesAccountProtocol {
    return CognitiveServicesAccountData()
  }
}

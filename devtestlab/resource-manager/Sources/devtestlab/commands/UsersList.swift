...
public protocol UsersList  {
    ...
    func execute(client: RuntimeClient,
        completionHandler: @escaping (ResponseWithContinuationUserProtocol?, Error?) -> Void) -> Void ;
}
...
internal class ListCommand : BaseCommand, UsersList {
    ...
    public init(subscriptionId: String, resourceGroupName: String, labName: String) {
        ...
        self.path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DevTestLab/labs/{labName}/users"
    ...
    public override func returnFunc(data: Data) throws -> Decodable? {
        ...
        if let mimeType = MimeType.getType(forStr: contentType) {
            ...
            let result = try decoder.decode(ResponseWithContinuationUserData?.self, from: data)
            if var pageDecoder = decoder as? PageDecoder {
    ...
    public func execute(client: RuntimeClient,
        ...
        client.executeAsync(command: self) {
            (result: ResponseWithContinuationUserData?, error: Error?) in

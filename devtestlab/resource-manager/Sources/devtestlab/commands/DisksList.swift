...
public protocol DisksList  {
    ...
    completionHandler: @escaping (ResponseWithContinuationDiskProtocol?, Error?) -> Void) -> Void ;
}

extension Commands.Disks {
// List list disks in a given user profile.
    internal class ListCommand : BaseCommand, DisksList {
        var nextLink: String?
        ...
        public init(subscriptionId: String, resourceGroupName: String, labName: String, userName: String) {
            ...
            self.path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DevTestLab/labs/{labName}/users/{userName}/disks"
        ...
        public override func returnFunc(data: Data) throws -> Decodable? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let result = try decoder.decode(ResponseWithContinuationDiskData?.self, from: data)
                if var pageDecoder = decoder as? PageDecoder {
        ...
        public func execute(client: RuntimeClient,
            ...
            client.executeAsync(command: self) {
                (result: ResponseWithContinuationDiskData?, error: Error?) in

...
public protocol SecretsList  {
    ...
    func execute(client: RuntimeClient,
        completionHandler: @escaping (ResponseWithContinuationSecretProtocol?, Error?) -> Void) -> Void ;
}

extension Commands.Secrets {
// List list secrets in a given user profile.
internal class ListCommand : BaseCommand, SecretsList {
    var nextLink: String?
    ...
    public init(subscriptionId: String, resourceGroupName: String, labName: String, userName: String) {
        ...
        self.path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DevTestLab/labs/{labName}/users/{userName}/secrets"
    ...
    public override func returnFunc(data: Data) throws -> Decodable? {
        ...
        if let mimeType = MimeType.getType(forStr: contentType) {
            ...
            let result = try decoder.decode(ResponseWithContinuationSecretData?.self, from: data)
            if var pageDecoder = decoder as? PageDecoder {
    ...
    public func execute(client: RuntimeClient,
        ...
        client.executeAsync(command: self) {
            (result: ResponseWithContinuationSecretData?, error: Error?) in

...
public protocol ArtifactsList  {
    ...
    func execute(client: RuntimeClient,
        completionHandler: @escaping (ResponseWithContinuationArtifactProtocol?, Error?) -> Void) -> Void ;
}

extension Commands.Artifacts {
// List list artifacts in a given artifact source.
internal class ListCommand : BaseCommand, ArtifactsList {
    var nextLink: String?
    ...
    public init(subscriptionId: String, resourceGroupName: String, labName: String, artifactSourceName: String) {
        ...
        self.path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DevTestLab/labs/{labName}/artifactsources/{artifactSourceName}/artifacts"
    ...
    public override func returnFunc(data: Data) throws -> Decodable? {
        ...
        if let mimeType = MimeType.getType(forStr: contentType) {
            ...
            let result = try decoder.decode(ResponseWithContinuationArtifactData?.self, from: data)
            if var pageDecoder = decoder as? PageDecoder {
    ...
    public func execute(client: RuntimeClient,
        ...
        client.executeAsync(command: self) {
            (result: ResponseWithContinuationArtifactData?, error: Error?) in

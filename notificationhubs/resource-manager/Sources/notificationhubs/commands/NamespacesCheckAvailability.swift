...
// useful because the domain name is created based on the service namespace name.
    internal class CheckAvailabilityCommand : BaseCommand, NamespacesCheckAvailability {
        public var subscriptionId : String
        public var apiVersion = "2017-04-01"
    public var parameters :  CheckAvailabilityParametersProtocol?

        public init(subscriptionId: String, parameters: CheckAvailabilityParametersProtocol) {
            self.subscriptionId = subscriptionId
            self.parameters = parameters
            super.init()
            self.method = "Post"
            self.isLongRunningOperation = false
            self.path = "/subscriptions/{subscriptionId}/providers/Microsoft.NotificationHubs/checkNamespaceAvailability"
        ...
        public override func encodeBody() throws -> Data? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let encodedValue = try encoder.encode(parameters as? CheckAvailabilityParametersData)
                return encodedValue
            }

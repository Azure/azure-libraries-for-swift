...
public protocol ServicePrincipalsListKeyCredentials  {
    ...
    completionHandler: @escaping (KeyCredentialListResultProtocol?, Error?) -> Void) -> Void ;
}
...
// ListKeyCredentials get the keyCredentials associated with the specified service principal.
    internal class ListKeyCredentialsCommand : BaseCommand, ServicePrincipalsListKeyCredentials {
        var nextLink: String?
        ...
        public override func returnFunc(data: Data) throws -> Decodable? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let result = try decoder.decode(KeyCredentialListResultData?.self, from: data)
                if var pageDecoder = decoder as? PageDecoder {
        ...
        public func execute(client: RuntimeClient,
            ...
            client.executeAsync(command: self) {
                (result: KeyCredentialListResultData?, error: Error?) in

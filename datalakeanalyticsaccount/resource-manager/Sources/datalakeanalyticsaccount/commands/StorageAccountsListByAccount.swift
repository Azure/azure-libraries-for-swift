...
public protocol StorageAccountsListByAccount  {
    ...
    completionHandler: @escaping (StorageAccountInformationListResultProtocol?, Error?) -> Void) -> Void ;
}
...
// account. The response includes a link to the next page, if any.
    internal class ListByAccountCommand : BaseCommand, StorageAccountsListByAccount {
        var nextLink: String?
        ...
        public init(subscriptionId: String, resourceGroupName: String, accountName: String) {
            ...
            self.path = "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DataLakeAnalytics/accounts/{accountName}/storageAccounts"
        ...
        public override func returnFunc(data: Data) throws -> Decodable? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let result = try decoder.decode(StorageAccountInformationListResultData?.self, from: data)
                if var pageDecoder = decoder as? PageDecoder {
        ...
        public func execute(client: RuntimeClient,
            ...
            client.executeAsync(command: self) {
                (result: StorageAccountInformationListResultData?, error: Error?) in

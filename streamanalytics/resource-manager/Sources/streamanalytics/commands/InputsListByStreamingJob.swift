...
// ListByStreamingJob lists all of the inputs under the specified streaming job.
    internal class ListByStreamingJobCommand : BaseCommand, InputsListByStreamingJob {
        var nextLink: String?
        ...
        public init(subscriptionId: String, resourceGroupName: String, jobName: String) {
            ...
            self.path = "/subscriptions/{subscriptionId}/resourcegroups/{resourceGroupName}/providers/Microsoft.StreamAnalytics/streamingjobs/{jobName}/inputs"
        ...
        public override func returnFunc(data: Data) throws -> Decodable? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let result = try decoder.decode(InputListResultData?.self, from: data)
                if var pageDecoder = decoder as? PageDecoder {
        ...
        public func execute(client: RuntimeClient,
            ...
            client.executeAsync(command: self) {
                (result: InputListResultData?, error: Error?) in

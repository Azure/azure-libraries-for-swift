...
public protocol JobListFromJobSchedule  {
    ...
    func execute(client: RuntimeClient,
        completionHandler: @escaping (CloudJobListResultProtocol?, Error?) -> Void) -> Void ;
}

extension Commands.Job {
// ListFromJobSchedule sends the list from job schedule request.
internal class ListFromJobScheduleCommand : BaseCommand, JobListFromJobSchedule {
    var nextLink: String?
    ...
    public init(jobScheduleId: String) {
        ...
        self.path = "/jobschedules/{jobScheduleId}/jobs"
    ...
    public override func returnFunc(data: Data) throws -> Decodable? {
        ...
        if let mimeType = MimeType.getType(forStr: contentType) {
            ...
            let result = try decoder.decode(CloudJobListResultData?.self, from: data)
            if var pageDecoder = decoder as? PageDecoder {
    ...
    public func execute(client: RuntimeClient,
        ...
        client.executeAsync(command: self) {
            (result: CloudJobListResultData?, error: Error?) in

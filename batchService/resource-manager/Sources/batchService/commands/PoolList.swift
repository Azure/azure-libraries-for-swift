...
public protocol PoolList  {
    ...
    completionHandler: @escaping (CloudPoolListResultProtocol?, Error?) -> Void) -> Void ;
}

extension Commands.Pool {
// List sends the list request.
    internal class ListCommand : BaseCommand, PoolList {
        var nextLink: String?
    ...
    public override init() {
        ...
        public override func returnFunc(data: Data) throws -> Decodable? {
            ...
            if let mimeType = MimeType.getType(forStr: contentType) {
                ...
                let result = try decoder.decode(CloudPoolListResultData?.self, from: data)
                if var pageDecoder = decoder as? PageDecoder {
        ...
        public func execute(client: RuntimeClient,
            ...
            client.executeAsync(command: self) {
                (result: CloudPoolListResultData?, error: Error?) in
